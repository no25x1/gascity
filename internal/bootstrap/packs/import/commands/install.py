...
recorded URL at the recorded commit, materializes each pack into
...
def main(argv: list[str]) -> int:
    ...
    # manifest should appear in the lock with a matching url. (We
    ...
    for handle, spec in spliced.imports.items():
        if spec.is_path():
            continue  # path imports don't have lock entries
        locked = lf.packs.get(handle)
        if locked is None:
            needs_resolve = True
            break
        if locked.url != spec.url:
            needs_resolve = True
            break
