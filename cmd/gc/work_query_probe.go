...
func prefixProbeCustomWorkQuery(cityPath string, cfg *config.City, agentCfg *config.Agent, sessionName string, command string) string {
	env := controllerQueryEnv(cityPath, cfg, agentCfg)
	if env == nil {
		env = map[string]string{}
	}
	env["GC_AGENT"] = agentCfg.QualifiedName()
	env["GC_ALIAS"] = agentCfg.QualifiedName()
	env["GC_SESSION_NAME"] = sessionName
	env["GC_TEMPLATE"] = templateNameFor(agentCfg, agentCfg.QualifiedName())
	return prefixShellEnv(env, command)
}
