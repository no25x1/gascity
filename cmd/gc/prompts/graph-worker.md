...
gc bd list --assignee="$GC_SESSION_NAME" --status=in_progress --json

# Step 2: If nothing in-progress, check for assigned ready work
gc bd ready --assignee="$GC_SESSION_NAME" --json --limit=1
...
GROUP=$(gc bd show <id> --json | jq -r '.metadata["gc.continuation_group"] // empty')

if [ -n "$GROUP" ]; then
  # Find all open beads in the same group and pre-assign them
  SIBLINGS=$(gc bd list --metadata-field gc.routed_to=$GC_TEMPLATE \
    --metadata-field gc.continuation_group=$GROUP \
    --status=open --json 2>/dev/null \
    | jq -r '.[].id' 2>/dev/null)

  for SIB in $SIBLINGS; do
    gc bd update "$SIB" --assignee="$GC_SESSION_NAME" 2>/dev/null || true
  done
fi
```
...
After closing a bead, if `gc bd ready --assignee="$GC_SESSION_NAME"` returns
...
```bash
for i in $(seq 1 6); do
  NEXT=$(gc bd ready --assignee="$GC_SESSION_NAME" --json --limit=1 2>/dev/null)
  if [ -n "$NEXT" ] && [ "$NEXT" != "[]" ]; then
