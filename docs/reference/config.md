...
| `gate` | string |  |  | Gate is a deprecated compatibility alias for Trigger. It is normalized away after decode so runtime code can stay trigger-only; writers that need rollback-safe persistence can mirror Trigger back here. |
